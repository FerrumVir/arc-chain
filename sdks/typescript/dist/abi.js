 ...
 */
function bytesToUint256(data, offset) {
    let val = 0n;
 ...
 */
function bytesToInt256(data, offset) {
 ...
 */
function splitTupleTypes(inner) {
    const result = [];
    let depth = 0;
    let current = "";
...
function encodeUint(value, bits) {
    ...
    if (v < 0n)
        throw new Error(`uint${bits} cannot be negative, got ${v}`);
...
function encodeInt(value, bits) {
    ...
    if (v < -half || v >= half) {
        throw new Error(`Value ${v} out of range for int${bits} [${-half}, ${half - 1n}]`);
    }
    if (v >= 0n)
        return uint256ToBytes(v);
    // Two's complement
...
function encodeBytesFixed(value, size) {
    let bytes;
    if (typeof value === "string") {
    ...
    if (bytes.length !== size) {
        throw new Error(`bytes${size} requires exactly ${size} bytes, got ${bytes.length}`);
    }
...
function encodeBytesDynamic(value) {
    let bytes;
    if (typeof value === "string") {
        bytes = hexToBytes(value);
    }
 ...
 */
function encodeSingle(typ, value) {
    if (typ === "address")
    ...
    if (typ === "bytes")
        return encodeBytesDynamic(value);
    let m = UINT_RE.exec(typ);
    if (m)
        return encodeUint(value, parseInt(m[1], 10));
    m = INT_RE.exec(typ);
    if (m)
        return encodeInt(value, parseInt(m[1], 10));
    m = BYTES_FIXED_RE.exec(typ);
    if (m)
        return encodeBytesFixed(value, parseInt(m[1], 10));
    ...
    if (m) {
        ...
        const arr = value;
        if (arr.length !== n) {
            throw new Error(`${typ} requires exactly ${n} elements, got ${arr.length}`);
        }
    ...
    if (m) {
        ...
        return encodeAbi(subtypes, value);
    }
    throw new Error(`Unsupported ABI type: ${typ}`);
}
function encodeArrayContents(innerType, items) {
...
function encodeAbi(types, values) {
    if (types.length !== values.length) {
        throw new Error(`types/values length mismatch: ${types.length} vs ${values.length}`);
    }
    ...
    for (let i = 0; i < types.length; i++) {
        if (dynamicFlags[i]) {
            ...
            const tailOffset = headSize + tailParts.reduce((sum, t) => sum + t.length, 0);
...
function decodeSingle(typ, data, offset, baseOffset) {
    if (typ === "address")
    ...
    let m = UINT_RE.exec(typ);
    if (m)
        return decodeUint(data, offset, parseInt(m[1], 10));
    m = INT_RE.exec(typ);
    if (m)
        return decodeInt(data, offset, parseInt(m[1], 10));
    m = BYTES_FIXED_RE.exec(typ);
    if (m)
        return decodeBytesFixed(data, offset, parseInt(m[1], 10));
    ...
    if (m) {
        ...
        const items = decodeArrayContents(innerType, count, data, absOffset + WORD_SIZE, absOffset + WORD_SIZE);
    ...
    if (m) {
        ...
        if (isDynamic(innerType)) {
            const tailOffset = Number(bytesToUint256(data, offset));
            const absOffset = baseOffset + tailOffset;
            const items = decodeArrayContents(innerType, n, data, absOffset, absOffset);
            return { value: items, nextOffset: offset + WORD_SIZE };
        }
        else {
            const items = decodeArrayContents(innerType, n, data, offset, baseOffset);
...
function decodeArrayContents(innerType, count, data, offset, baseOffset) {
    const results = [];
    let cursor = offset;
    for (let i = 0; i < count; i++) {
        const { value, nextOffset } = decodeSingle(innerType, data, cursor, baseOffset);
        results.push(value);
        cursor = nextOffset;
    }
    return results;
}
function decodeTuple(types, data, offset, baseOffset) {
    const results = [];
    let cursor = offset;
...
function encodeFunctionCall(signature, ...args) {
    ...
    if (args.length !== paramTypes.length) {
        throw new Error(`Signature ${signature} expects ${paramTypes.length} args, got ${args.length}`);
    }
 ...
 * @returns Array of decoded values
 */
function decodeFunctionResult(types, data) {
 ...
 */
function decodeFunctionInput(signature, data) {
    ...
    if (bytesToHex(expectedSelector) !== bytesToHex(actualSelector)) {
        throw new Error(`Selector mismatch: expected ${bytesToHex(expectedSelector)}, got ${bytesToHex(actualSelector)}`);
    }
