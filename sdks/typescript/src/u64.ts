...
export function u64ToBigInt(value: U64, fieldName: string): bigint {
  if (typeof value === "number") {
    if (!Number.isSafeInteger(value) || value < 0) {
      throw new RangeError(
        `${fieldName} must be a non-negative safe integer or bigint in the u64 range`,
      );
    }
    return BigInt(value);
  }
...
export function assertU64(value: U64, fieldName: string): void {
...
export function stringifyJsonWithBigInts(value: unknown): string {
  const serialized = serializeJsonValue(value, new Set<object>());
 ...
 */
export function parseJsonWithBigInts<T>(text: string): T {
  let index = 0;
  ...
  const parseString = (): string => {
    if (text[index] !== '"') return syntaxError("expected string");
    const start = index;
    index += 1;
    while (index < text.length) {
      ...
      if (character === "\\") {
        if (index >= text.length) return syntaxError("unterminated escape");
        const escape = text[index];
        index += 1;
        if (escape === "u") {
          const hex = text.slice(index, index + 4);
          if (!/^[0-9a-fA-F]{4}$/.test(hex)) return syntaxError("invalid unicode escape");
          index += 4;
  ...
  const parseNumber = (fieldName: string): number | bigint => {
    const start = index;
    ...
    if (text[index] === ".") {
      index += 1;
      if (!/[0-9]/.test(text[index] ?? "")) return syntaxError("invalid fraction");
      while (/[0-9]/.test(text[index] ?? "")) index += 1;
    }
    if (text[index] === "e" || text[index] === "E") {
      index += 1;
      if (text[index] === "+" || text[index] === "-") index += 1;
      if (!/[0-9]/.test(text[index] ?? "")) return syntaxError("invalid exponent");
      while (/[0-9]/.test(text[index] ?? "")) index += 1;
    }
    ...
    if (isIntegerToken) {
      if (token.startsWith("-")) {
        ...
        if (!Number.isSafeInteger(signed)) {
          throw new RangeError(
            `${fieldName || "RPC value"} exceeds JavaScript's safe-integer range`,
          );
        }
        return signed;
      }
      ...
      if (exact <= BigInt(Number.MAX_SAFE_INTEGER)) return Number(exact);
      if (!U64_RESPONSE_FIELDS.has(fieldName)) {
        throw new RangeError(
          `${fieldName || "RPC value"} exceeds JavaScript's safe-integer range`,
        );
      }
      if (exact > U64_MAX) throw new RangeError(`${fieldName} exceeds the u64 range`);
      return exact;
    }
    ...
    if (!Number.isFinite(numeric) || (Number.isInteger(numeric) && !Number.isSafeInteger(numeric))) {
      throw new RangeError(
        `${fieldName || "RPC value"} was not an exact safe number in the RPC response`,
      );
    }
    if (
      U64_RESPONSE_FIELDS.has(fieldName) &&
  ...
  const parseValue = (fieldName: string): unknown => {
    skipWhitespace();
    const character = text[index];
    ...
    if (character === "[") {
      ...
      while (true) {
        ...
        if (text[index] !== ",") return syntaxError("expected ',' or ']'");
        index += 1;
      }
    }
    if (character === "{") {
      ...
      while (true) {
        ...
        skipWhitespace();
        if (text[index] !== ":") return syntaxError("expected ':'");
        index += 1;
  ...
  skipWhitespace();
  if (index !== text.length) syntaxError("unexpected trailing input");
  return result as T;
}
  ...
  switch (typeof value) {
    ...
    case "number":
      if (Number.isInteger(value) && !Number.isSafeInteger(value)) {
        throw new RangeError(
          "JSON integer must be a safe number; use bigint for larger exact values",
        );
      }
