...
signature_count=0
for signature_name in "${UPDATER_SIGNATURES[@]}"; do
    signature_path="$RELEASE_FILES_DIR/$signature_name"
...
while IFS= read -r -d '' signature_path; do
    ...
    case "$signature_name" in
        ...
        arc-desktop-linux-x86_64.AppImage.sig|\
        SHA256SUMS.sig) ;;
        *) die "unexpected published signature artifact: $signature_name" ;;
    esac
done < <(find "$RELEASE_FILES_DIR" -maxdepth 1 -type f -name '*.sig' -print0)
