...
use arc_crypto::stwo_air::{try_prove_dense_stark, try_prove_dense_packed};

fn data(seed: &str, len: usize) -> Vec<i64> {
    let mut r: u64 = 0;
    for b in seed.bytes() { r = r.wrapping_mul(31).wrapping_add(b as u64); }
    (0..len).map(|_| { r = r.wrapping_mul(6364136223846793005).wrapping_add(1442695040888963407);
        ((r >> 33) as i64) % 10 - 5 }).collect()
}
