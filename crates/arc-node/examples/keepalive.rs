...
async fn main() {
    ...
    loop {
        let acc: Value = match c.get(format!("{}/account/{}", COORD, from_hex)).send().await {
        ...
        if bal < 100 { let _ = c.post(format!("{}/faucet/claim", COORD)).json(&json!({"address": &from_hex})).send().await; tokio::time::sleep(Duration::from_secs(10)).await; continue; }
        let mut tx = Transaction {
            tx_type: TxType::Transfer, from, nonce,
        ...
        let _ = c.post(format!("{}/tx/submit_signed", COORD)).json(&tx).send().await;
