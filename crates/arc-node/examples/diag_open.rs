...
async fn main() {
    ...
    let phrase=format!("diag-open-{}",std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs());
    let (sk,pk,from)=keypair(&phrase);
    let from_hex=hex::encode(from.0);
    let _=c.post(format!("{}/faucet/claim",COORD)).json(&json!({"address":&from_hex})).send().await;
    ...
    let mut request_id=[0u8;32]; rand::RngCore::fill_bytes(&mut rand::rngs::OsRng,&mut request_id);
    let body=InferenceEscrowOpenBody {
        request_id, model_id:hash_bytes(b"test-model"),
        max_fee:1000, max_tokens:3, timeout_blocks:10000,
    };
    let mut tx=Transaction {
        tx_type:TxType::InferenceEscrowOpen, from, nonce,
        body:TxBody::InferenceEscrowOpen(body),
        fee:0, gas_limit:0, hash:Hash256::ZERO,
        signature:Signature::null(), sig_verified:false,
    };
    ...
    println!("tx JSON ({} chars): {}",json_body.len(),&json_body[..json_body.len().min(300)]);
    ...
    for i in 0..30 {
        ...
        let info:Value=c.get(format!("{}/info",COORD)).send().await.unwrap().json().await.unwrap();
        let height=info.get("block_height").and_then(|x|x.as_u64()).unwrap_or(0);
        let mp=info.get("mempool_size").and_then(|x|x.as_u64()).unwrap_or(0);
        let txr=c.get(format!("{}/tx/0x{}",COORD,h)).send().await;
        let txs=match txr {Ok(r)=>format!("{}",r.status()),Err(e)=>format!("err:{}",e)};
        let acc:Value=c.get(format!("{}/account/{}",COORD,from_hex)).send().await.unwrap().json().await.unwrap();
        let n=acc.get("nonce").and_then(|n|n.as_u64()).unwrap_or(0);
        println!("[{}s] h={} mp={} tx={} from.n={}",(i+1)*2,height,mp,txs,n);
        if n>nonce {println!("✓ EXEC"); break;}
    }
}
