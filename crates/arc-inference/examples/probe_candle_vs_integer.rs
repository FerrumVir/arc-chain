...
fn main() {
    ...
    let candle_logits = if candle_logits.dims().len() == 2 {
        candle_logits
            .get(candle_logits.dim(0).unwrap() - 1)
            .unwrap()
    } else {
        candle_logits
    };
    ...
    eprintln!(
        "  block_i8_output installed: {}",
        integer_model.block_i8_output.is_some()
    );
    ...
    let candle_top100: std::collections::HashSet<usize> =
        top_k(&candle_vec, 100).iter().map(|(i, _)| *i).collect();
    ...
    let c_rank: std::collections::HashMap<usize, usize> = top_k(&candle_vec, candle_vec.len())
        ...
        .collect();
    let our_rank: std::collections::HashMap<usize, usize> =
        top_k_i64(&our_logits, our_logits.len())
            .iter()
            .enumerate()
            .map(|(rank, (idx, _))| (*idx, rank))
            .collect();
    ...
    let mean_rank_diff: f64 = top_k(&candle_vec, 100)
        .iter()
        .map(|(idx, _)| {
