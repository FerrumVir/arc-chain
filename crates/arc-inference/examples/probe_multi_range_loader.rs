...
fn summarize(tag: &str, layers_loaded: &[bool], per_layer_bytes: &[usize]) {
    ...
    let loaded: Vec<usize> = layers_loaded.iter().enumerate()
        .filter_map(|(i, &b)| if b { Some(i) } else { None })
        .collect();
    eprintln!(
        "{tag}: {} layers loaded {:?}, {} bytes total",
        loaded.len(), loaded, total
    );
}
...
fn main() {
    ...
    eprintln!("A) loading ranges {:?} via load_cached_model_ranges...", ranges);
    let merged = load_cached_model_ranges(MODEL_PATH, &ranges)
        .expect("load_cached_model_ranges failed");
    ...
    let expected: Vec<bool> = (0..n).map(|i|
        ranges.iter().any(|&(s, e)| i >= s && i < e)
    ).collect();
    let mismatches: Vec<usize> = (0..n).filter(|&i| merged_flags[i] != expected[i]).collect();
    assert!(mismatches.is_empty(), "Merged layers disagree with expected union at {:?}", mismatches);
    ...
    if covers_tail {
        assert!(merged.output_weight.n_rows != 0, "output head must load when range ends at n_layers");
    } else {
        assert!(merged.output_weight.n_rows == 0, "output head must NOT load when no range ends at n_layers");
    }
    ...
    for &(s, e) in &ranges {
        ...
        let single = load_cached_model_shard(MODEL_PATH, s, e)
            .expect("load_cached_model_shard failed");
        ...
        for i in s..e {
            ...
            assert_eq!(a.w_down.n_rows, b.w_down.n_rows, "w_down.n_rows differs at layer {i}");
            assert_eq!(layer_bytes(a), layer_bytes(b), "layer {i} byte size differs");
            ...
            assert_eq!(a.w_down.data, b.w_down.data, "w_down bytes differ at layer {i}");
        }
