...
fn main() {
    ...
    let path = args
        .get(1)
        .expect("Usage: bench_gpu_forward <model.arc-int8|model.gguf>");
    ...
    let layer_data: Vec<_> = model
        .layers
        .iter()
        .map(|l| {
            (
                l.wq.data.as_slice(),
                l.wq.scales.as_slice(),
                l.wk.data.as_slice(),
                l.wk.scales.as_slice(),
                l.wv.data.as_slice(),
                l.wv.scales.as_slice(),
                l.wo.data.as_slice(),
                l.wo.scales.as_slice(),
                l.w_gate.data.as_slice(),
                l.w_gate.scales.as_slice(),
                l.w_up.data.as_slice(),
                l.w_up.scales.as_slice(),
                l.w_down.data.as_slice(),
                l.w_down.scales.as_slice(),
    ...
    let gpu_model = gpu.upload_model(
        &model.embedding_i8.data,
        &model.embedding_i8.scales,
        &model.output_weight.data,
        &model.output_weight.scales,
        &model.final_norm,
        &layer_data,
        &cfg.rope_cos,
        &cfg.rope_sin,
    ...
    println!(
        "Speedup: {:.1}x",
        cpu_elapsed.as_secs_f64() / elapsed2.as_secs_f64().max(0.0001)
    );
}
