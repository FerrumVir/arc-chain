...
fn main() {
    let model = load_cached_model(
        &std::env::args()
            .nth(1)
            .unwrap_or("/tmp/tinyllama-1.1b-chat.Q8_0.gguf".into()),
    )
    .unwrap();
    ...
    println!(
        "range=[{}, {}] mean={}",
        hidden.iter().min().unwrap(),
        hidden.iter().max().unwrap(),
    ...
    println!(
        "normed range=[{}, {}]",
        normed.iter().min().unwrap(),
        normed.iter().max().unwrap()
    );
    ...
    for x_f in [-2.0f64, -1.0, -0.5, 0.0, 0.5, 1.0, 2.0] {
        ...
        println!(
            "  SiLU({:.1}) = int:{:.4} real:{:.4} (err={:.2}%)",
            x_f,
            result_int as f64 / ONE as f64,
            result_real,
