...
fn main() {
    ...
    if args.len() < 2 {
        eprintln!(
            "Usage: {} <model.gguf|model.arc-int8> [num_tokens] [--save path]",
            args[0]
        );
        std::process::exit(1);
    }
    ...
    println!(
        "Config: {} layers, d_model={}, n_heads={}, n_kv_heads={}, d_ff={}, vocab={}",
    ...
    println!(
        "Generated {} tokens in {}ms ({} ms/token)",
        n_gen, total_ms, ms_per_token
    );
    ...
    if hash == hash2 && generated == gen2 {
        println!(
            "PASS: Two runs produce identical output (hash: 0x{})",
            hex::encode(&hash.0[..8])
        );
    } else {
        println!(
            "FAIL: Non-deterministic! hash1={}, hash2={}",
            hex::encode(&hash.0[..8]),
    ...
    if all_match {
        println!(
            "PASS: 7 consecutive runs all produce hash 0x{}",
            hex::encode(&hash.0[..8])
        );
    }
    ...
    println!(
        "Speed: {} ms/token ({} tokens in {}ms)",
        ms_per_token, n_gen, total_ms
    );
