...
use arc_inference::cached_integer_model::{CachedIntegerModel, KVCache, load_cached_model};
...
fn summarize(label: &str, logits: &[i64]) {
    ...
    eprintln!(
        "{:35} argmax={:5} max_abs={:8} mean_abs={:6}",
        label, argmax, max_abs, mean_abs
    );
}
...
fn main() {
    ...
    let saved_out = model.i16_output.take();
    summarize(
        "I8 layers + I8 output (baseline)",
        &forward_token(&model, 5000),
    );
    model.i16_layers = saved_layers;
    model.i16_output = saved_out;
}
