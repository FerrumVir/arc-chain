...
fn main() {
    ...
    let path = args
        .get(1)
        .expect("Usage: bench_q4_gpu <model.arc-int8|model.gguf>");
    ...
    println!(
        "Q4 conversion (3 matrices): {:.1} ms",
        q4_start.elapsed().as_secs_f64() * 1000.0
    );
    println!(
        "Memory: Wq {} KB (was {} KB), W_gate {} KB (was {} KB)",
        q4_wq.memory_bytes() / 1024,
        model.layers[0].wq.memory_bytes() / 1024,
        q4_gate.memory_bytes() / 1024,
        model.layers[0].w_gate.memory_bytes() / 1024
    );

    // Create test input
    let d = cfg.d_model;
    let dff = cfg.d_ff;
    let input_d: Vec<i64> = (0..d)
        .map(|i| (i as i64 % 200 - 100) * 65536 / 100)
        .collect();
    ...
    let start = Instant::now();
    for _ in 0..iters {
        matmul_q4_preq(&q4_wq, &input_d_q, d, &mut out_d);
    }
    let q4_wq_preq = start.elapsed().as_secs_f64() * 1000.0 / iters as f64;

    println!(
        "  Wq  [{}×{}]: INT8 {:.2}ms, Q4 {:.2}ms, Q4+preq {:.2}ms ({:.1}x)",
        d,
        d,
        i8_wq,
        q4_wq_ms,
        q4_wq_preq,
        i8_wq / q4_wq_preq.max(0.001)
    );

    // INT8 W_gate
    let start = Instant::now();
    for _ in 0..iters {
        let _ = matmul_fast(&model.layers[0].w_gate, &input_d, d, dff);
    }
    ...
    let start = Instant::now();
    for _ in 0..iters {
        matmul_q4_preq(&q4_gate, &input_d_q, d, &mut out_ff);
    }
    let q4_gate_preq = start.elapsed().as_secs_f64() * 1000.0 / iters as f64;

    println!(
        "  gate[{}×{}]: INT8 {:.2}ms, Q4+preq {:.2}ms ({:.1}x)",
        dff,
        d,
        i8_gate,
        q4_gate_preq,
        i8_gate / q4_gate_preq.max(0.001)
    );

    // INT8 W_down
    let start = Instant::now();
    for _ in 0..iters {
        let _ = matmul_fast(&model.layers[0].w_down, &input_ff, dff, d);
    }
    ...
    let start = Instant::now();
    for _ in 0..iters {
        matmul_q4_preq(&q4_down, &input_ff_q, dff, &mut out_d2);
    }
    let q4_down_preq = start.elapsed().as_secs_f64() * 1000.0 / iters as f64;

    println!(
        "  down[{}×{}]: INT8 {:.2}ms, Q4+preq {:.2}ms ({:.1}x)",
        d,
        dff,
        i8_down,
        q4_down_preq,
        i8_down / q4_down_preq.max(0.001)
    );
    ...
    println!(
        ...
        total_i8_est,
        total_q4_est,
        total_i8_est / total_q4_est.max(0.001)
    );
    ...
    match arc_gpu::gpu_matmul::GpuMatmul::new(dff, cfg.vocab_size) {
        Ok(gpu) => {
            ...
            println!(
                "  GPU Wq [{}×{}]: {:.2}ms ({:.1}x vs INT8 CPU)",
                d,
                d,
                gpu_ms,
                i8_wq / gpu_ms.max(0.001)
            );
        }
