    ...
    impl MetalDirectForward {
        ...
        pub fn new(
            ...
            let matmul_lib = device
                .new_library_with_source(include_str!("matmul.metal"), &compile_opts)
                .map_err(|e| format!("matmul.metal: {e}"))?;
            let fused_lib = device
                .new_library_with_source(include_str!("fused_kernels.metal"), &compile_opts)
                .map_err(|e| format!("fused_kernels.metal: {e}"))?;

            // UNTESTED - new shader libraries for complete forward pass
            let rope_lib = device
                .new_library_with_source(include_str!("rope.metal"), &compile_opts)
                .map_err(|e| format!("rope.metal: {e}"))?;
            let attn_lib = device
                .new_library_with_source(include_str!("attention.metal"), &compile_opts)
                .map_err(|e| format!("attention.metal: {e}"))?;
            let silu_lib = device
                .new_library_with_source(include_str!("silu.metal"), &compile_opts)
                .map_err(|e| format!("silu.metal: {e}"))?;
            let residual_lib = device
                .new_library_with_source(include_str!("residual.metal"), &compile_opts)
                .map_err(|e| format!("residual.metal: {e}"))?;
            let argmax_lib = device
                .new_library_with_source(include_str!("argmax.metal"), &compile_opts)
                .map_err(|e| format!("argmax.metal: {e}"))?;

            let matmul_fn = matmul_lib
                .get_function("matmul_i8", None)
                .map_err(|e| format!("matmul_i8: {e}"))?;
            let q4_fn = fused_lib
                .get_function("matmul_i4", None)
                .map_err(|e| format!("matmul_i4: {e}"))?;
            let lnq_fn = fused_lib
                .get_function("layernorm_quantize", None)
                .map_err(|e| format!("layernorm_quantize: {e}"))?;
            let rope_fn = rope_lib
                .get_function("rope_apply", None)
                .map_err(|e| format!("rope_apply: {e}"))?;
            let attn_fn = attn_lib
                .get_function("attention", None)
                .map_err(|e| format!("attention: {e}"))?;
            let silu_fn = silu_lib
                .get_function("silu_mul", None)
                .map_err(|e| format!("silu_mul: {e}"))?;
            let residual_fn = residual_lib
                .get_function("residual_add", None)
                .map_err(|e| format!("residual_add: {e}"))?;
            let argmax_fn = argmax_lib
                .get_function("argmax_i32", None)
                .map_err(|e| format!("argmax_i32: {e}"))?;

            let matmul_pso = device
                .new_compute_pipeline_state_with_function(&matmul_fn)
                .map_err(|e| format!("matmul PSO: {e}"))?;
            let matmul_q4_pso = device
                .new_compute_pipeline_state_with_function(&q4_fn)
                .map_err(|e| format!("Q4 PSO: {e}"))?;
            let fused_lnq_pso = device
                .new_compute_pipeline_state_with_function(&lnq_fn)
                .map_err(|e| format!("LNQ PSO: {e}"))?;
            let rope_pso = device
                .new_compute_pipeline_state_with_function(&rope_fn)
                .map_err(|e| format!("RoPE PSO: {e}"))?;
            let attention_pso = device
            ...
            let residual_pso = device
                .new_compute_pipeline_state_with_function(&residual_fn)
                .map_err(|e| format!("residual PSO: {e}"))?;
            let argmax_pso = device
                .new_compute_pipeline_state_with_function(&argmax_fn)
                .map_err(|e| format!("argmax PSO: {e}"))?;

            info!(
                "Metal direct: 8 compute pipelines compiled (matmul_i8, matmul_i4, fused_lnq, rope, attention, silu, residual, argmax)"
            );
        ...
        pub fn dispatch_rope(
            ...
            encoder.dispatch_thread_groups(MTLSize::new(tg_count, 1, 1), MTLSize::new(256, 1, 1));
        }
        ...
        pub fn forward_token(
            ...
            for (layer_idx, lw) in layer_weights.iter().enumerate() {
                ...
                self.dispatch_fused_lnq(
                    encoder,
                    &model.hidden_buf,
                    &model.normed_packed_buf,
                    &lw.attn_norm,
                    &model.ln_params_buf,
                    &model.quant_scale_buf,
                );
