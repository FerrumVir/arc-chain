...
impl GpuModel {
    pub fn normed_packed_ref(&self) -> &wgpu::Buffer {
        &self.normed_packed
    }
    pub fn q_buf_ref(&self) -> &wgpu::Buffer {
        &self.q_buf
    }
    pub fn quant_scale_ref(&self) -> &wgpu::Buffer {
        &self.quant_scale
    }
    pub fn normed_buf_ref(&self) -> &wgpu::Buffer {
        &self.normed_buf
    }
}
...
impl GpuForward {
    ...
    pub fn new() -> Result<Self, String> {
        ...
        info!(
            "GPU forward: {} ({:?}), msl={}",
            info.name, info.backend, has_msl
        );
        ...
        let (msl_fused_lnq_pipeline, msl_matmul_q4_pipeline) = if has_msl {
            ...
            let fused_lnq = {
                ...
                let pl = device.create_pipeline_layout(&wgpu::PipelineLayoutDescriptor {
                    label: None,
                    bind_group_layouts: &[&fused_lnq_bgl],
                    push_constant_ranges: &[],
                });
                match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
                    device.create_compute_pipeline(&wgpu::ComputePipelineDescriptor {
                        label: Some("fused_lnq_msl"),
                        layout: Some(&pl),
                        module: &msl_shader,
                        entry_point: Some("layernorm_quantize"),
                        compilation_options: Default::default(),
                        cache: None,
                    })
                })) {
        ...
        let n_msl = [
            &msl_matmul_pipeline,
            &msl_fused_lnq_pipeline,
            &msl_matmul_q4_pipeline,
        ]
        .iter()
        .filter(|p| p.is_some())
        .count();
        ...
        info!(
            "All {} GPU compute pipelines compiled ({} WGSL + {} Metal)",
            n_wgsl + n_msl,
            n_wgsl,
            n_msl
        );
    ...
    pub fn matmul_bgl_ref(&self) -> &wgpu::BindGroupLayout {
        &self.matmul_bgl
    }
    pub fn fused_lnq_bgl_ref(&self) -> &wgpu::BindGroupLayout {
        &self.fused_lnq_bgl
    }
    ...
    pub fn upload_model(
        ...
        for (i, l) in layers.iter().enumerate() {
            ...
            if i % 8 == 0 {
                info!("Layer {}/{} uploaded", i + 1, layers.len());
            }
        }
        ...
        let hidden_buf = self.empty_buf(
            "hidden",
            i32_size(d_model),
            sto_rw | wgpu::BufferUsages::COPY_DST,
        );
        ...
        let logits_buf = self.empty_buf(
            "logits",
            i32_size(vocab_size),
            sto_rw | wgpu::BufferUsages::COPY_SRC,
        );
        let result_buf = self.empty_buf("result", 4, sto_rw | wgpu::BufferUsages::COPY_SRC);
        let staging_buf = self.empty_buf(
            "staging",
            4,
            wgpu::BufferUsages::MAP_READ | wgpu::BufferUsages::COPY_DST,
        );
        let kv_k_bufs: Vec<_> = (0..n_layers)
            .map(|i| self.empty_buf(&format!("kv_k{i}"), u32_size(max_seq * d_kv), sto_rw))
            .collect();
        let kv_v_bufs: Vec<_> = (0..n_layers)
            .map(|i| self.empty_buf(&format!("kv_v{i}"), u32_size(max_seq * d_kv), sto_rw))
            .collect();
        let kv_k_scales: Vec<_> = (0..n_layers)
            .map(|i| self.empty_buf(&format!("ks{i}"), i32_size(max_seq), sto_rw))
            .collect();
        let kv_v_scales: Vec<_> = (0..n_layers)
            .map(|i| self.empty_buf(&format!("vs{i}"), i32_size(max_seq), sto_rw))
            .collect();
        let rope_cos_buf = self.buf("cos", &pack_i64(rope_cos), sto);
        let rope_sin_buf = self.buf("sin", &pack_i64(rope_sin), sto);

        info!(
            "GPU model uploaded: {} layers, d={}, vocab={}",
        ...
        let ln_params_buf = self.buf(
            "ln_p",
            bytemuck::bytes_of(&ln_params),
            wgpu::BufferUsages::UNIFORM,
        );
        let mm_q_params = self.buf(
            "mm_q_p",
            bytemuck::bytes_of(&MatmulParams {
                in_size: d_model,
                out_size: d_model,
                scale_offset: 0,
                _pad: 0,
        ...
        // Shared param buffers (all layers use the same - updated once per token)
        let shared_rqp = self.buf(
            "shared_rqp",
            bytemuck::bytes_of(&RopeParams {
                pos: 0,
                d_head,
                n_heads,
                _pad: 0,
        ...
        let shared_atp = self.buf(
            "shared_atp",
            bytemuck::bytes_of(&AttnParams {
                d_head,
                n_heads,
                n_kv_heads,
                seq_len: 1,
        ...
        for i in 0..n_layers as usize {
            ...
            layer_bgs_vec.push(LayerBindGroups {
                fused_lnq_attn: mk_bg(
                    &[
                        be(0, &hidden_buf),
                        be(1, &normed_packed),
                        be(2, &layer_attn_norm[i]),
                        be(3, &ln_params_buf),
                        be(4, &quant_scale),
                    ],
                    &self.fused_lnq_bgl,
                ...
                residual_attn: mk_bg(
                    &[be(0, &hidden_buf), be(1, &projected_buf)],
                    &self.residual_bgl,
                ),
                ln_ffn: mk_bg(
                    &[
                ...
                quantize_gated: mk_bg(
                    &[be(0, &gate_buf), be(1, &normed_packed), be(2, &quant_scale)],
                    &self.quantize_bgl,
                ),
                mm_down: mk_bg(
                    &[
                ...
                residual_ffn: mk_bg(
                    &[be(0, &hidden_buf), be(1, &ff_out_buf)],
                    &self.residual_bgl,
                ),
            });
        }

        let fused_final_lnq_bg = mk_bg(
            &[
                be(0, &hidden_buf),
                be(1, &normed_packed),
                be(2, &final_norm_buf),
        ...
        let lm_head_bg = mk_bg(
            &[
                be(0, &output_weight_buf),
                be(1, &normed_packed),
                be(2, &logits_buf),
                be(3, &mm_lm_params),
                be(4, &output_scales),
        ...
        info!(
            "All bind groups pre-created ({} per layer × {} layers + 4 final = {} total)",
            19,
            n_layers,
            19 * n_layers + 4
        );
        ...
        let emb_i32: Vec<i32> = embedding_data
            .iter()
            .zip((0..embedding_data.len()).map(|i| {
                let row = i / (d_model as usize);
                if row < embedding_scales.len() {
                    embedding_scales[row]
                } else {
                    1
                }
            }))
...
impl GpuForward {
    ...
    pub fn forward_one_token(&self, model: &GpuModel, token: u32, seq_pos: u32) -> u32 {
        ...
        self.queue
            .write_buffer(&model.hidden_buf, 0, bytemuck::cast_slice(emb));
        ...
        self.queue
            .write_buffer(&model.rope_q_params, 0, bytemuck::bytes_of(&rp));
        let rk = RopeParams {
            pos: seq_pos,
            d_head: dh,
        ...
        self.queue
            .write_buffer(&model.rope_k_params, 0, bytemuck::bytes_of(&rk));
        let ap = AttnParams {
            d_head: dh,
            n_heads: model.n_heads,
            ...
            seq_len: seq_pos + 1,
            d_kv: model.d_kv,
            attn_scale: model.attn_scale,
            _p1: 0,
            _p2: 0,
        };
        self.queue
            .write_buffer(&model.attn_params_bufs, 0, bytemuck::bytes_of(&ap));
        ...
        if std::env::var("GPU_PROFILE").is_ok() {
            eprintln!(
                "GPU fwd: params={:.1}ms encode={:.1}ms submit={:.1}ms poll={:.1}ms total={:.1}ms",
