...
fn main() {
    ...
    let prompt: Vec<u32> = if is_llama3 {
        ...
        vec![
            1, 518, 25580, 29962, 1724, 338, 29871, 29906, 29974, 29906, 29973, 518, 29914, 25580,
            29962,
        ]
    };
    let eos: Vec<u32> = if is_llama3 {
        vec![128001, 128009]
    } else {
        vec![2]
    };
    ...
    eprintln!(
        "  Model loaded in {:.1}s (one-time cost)",
        load_time.as_secs_f64()
    );
    eprintln!(
        "  Config: {} layers, d_model={}, {} heads, d_ff={}, vocab={}",
    ...
    for run in 0..runs {
        ...
        eprintln!(
            "  Run {}: {} tokens in {:.1}s ({:.0}ms/tok), hash={}",
            run,
            tokens.len(),
            elapsed.as_secs_f64(),
            per_token,
