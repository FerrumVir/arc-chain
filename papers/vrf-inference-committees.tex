...
\begin{itemize}
    \item \textbf{Fast finality}: Results are confirmed in one consensus round ($\sim$12ms per round, $\sim$24ms 2-round finality) instead of waiting for a multi-block challenge period.
...
\begin{enumerate}
    ...
    \item Committee selection and vote collection occur within a single consensus round ($\sim$12ms), leaving insufficient time for targeted corruption.
...
\begin{itemize}
    ...
    \item \textbf{2-round finality} ($\sim$24ms measured), enabling committee results to be confirmed within a single consensus cycle.
\end{itemize}
