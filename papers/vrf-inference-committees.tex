...
We implement this protocol in ARC Chain, a sender-sharded DAG blockchain with 24 native transaction types. Our implementation includes a four-tier hardware registry where validators declare their inference capabilities, an EIP-1559-style separate gas market for inference transactions, and per-address rate limiting. Security analysis shows that with 10\% malicious validators and a 7-member committee requiring 5/7 agreement, the probability of corruption is 0.018\%. We provide a complete implementation with 1,128 passing tests.
\end{abstract}
...
\begin{itemize}
    \item \textbf{Fast finality}: Results are confirmed in one consensus round ($\sim$4.3s at current block times) instead of waiting for a multi-block challenge period.
...
To our knowledge, this is the first protocol to combine VRF-based random committee selection with tiered AI inference capabilities in a production blockchain. We implement the full system in Rust (80,683 lines of code, 14 crates, 1,185 tests) as part of ARC Chain, a Layer 1 blockchain designed for AI agent coordination.
...
After $n$ consecutive full blocks, the fee is $\text{base} \times (9/8)^n$. After 20 full blocks, the fee is $\sim$10.5$\times$ the base; after 50, $\sim$361$\times$. Combined with the stake requirement for registration, sustained computational attacks are economically infeasible.
...
At the BFT threshold of $f < 1/3$, the corruption probability is 4.34\%, which remains tractable given the economic stakes involved. For the more realistic scenario of $f = 10\%$ (given that inference providers must stake significant capital), the probability drops to 0.018\%.
...
\begin{enumerate}
    ...
    \item Committee selection and vote collection occur within a single consensus round ($\sim$4.3s), leaving insufficient time for targeted corruption.
...
We implement the VRF inference committee protocol in ARC Chain, a sender-sharded DAG blockchain written in Rust. The implementation spans 14 crates totaling 80,683 lines of code with 1,128 passing tests.
...
At a base fee of 100,000 ARC and block time of $\sim$2 seconds, sustaining an attack for 100 blocks (200 seconds) would cost over 13 billion ARC per transaction---an economically irrational expenditure.
...
\begin{itemize}
    ...
    \item \textbf{2-round finality} ($\sim$4.3s at current block times), enabling committee results to be confirmed within a single consensus cycle.
\end{itemize}
