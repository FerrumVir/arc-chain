...
pub fn inclusive_prefix_sum(
    ...
    for ([l0, l1], [r0, r1]) in izip!(l_half.as_chunks_mut().0.iter_mut(), r_half.as_chunks_mut().0.iter_mut().rev()) {
    ...
    while chunk_size > 1 {
        ...
        zip(lows.as_chunks_mut().0.iter_mut(), highs.as_chunks().0.iter())
    ...
    *half_coset0_sums.first_mut().unwrap() = first_vec.into();
    ...
    while chunk_size < half_coset0_sums.len() {
        let (lows, highs) = half_coset0_sums.split_at_mut(chunk_size);
        zip(lows.as_chunks_mut().0.iter_mut(), highs.as_chunks_mut().0.iter_mut())
