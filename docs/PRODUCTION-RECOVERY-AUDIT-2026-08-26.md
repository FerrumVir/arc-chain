...
helper. Root-pinned archive metadata fetches use an exact private mode-`0600`
rclone-config copy under an isolated disposable `HOME`; OAuth refresh and cache
writes are removed with that root on success or failure, and the original
operator config is re-proved afterward. The rollout requires an exact
archive-bound GO phrase before any persistent recovery-managed mutation,
imports only into fresh or exact same-manifest resumable data directories,
checks H/H+1 continuity and restart convergence, and can require a successful
